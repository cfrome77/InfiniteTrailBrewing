...
import { getSubscriberCount, sendNewsletter } from "@/app/actions/send-newsletter";
        ...
        <div className="space-y-8">
            <div className="grid md:grid-cols-2 gap-6">
                ...
                <Card className="border-tan/20">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium">Newsletter Status</CardTitle>
                        <Mail className="h-4 w-4 text-forest/50" />
                    </CardHeader>
                    <CardContent>
                        {status ? (
                            <div className={`flex items-start gap-2 p-3 rounded-md text-sm ${status.success ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>
                                {status.success ? <CheckCircle2 className="h-4 w-4 mt-0.5 shrink-0" /> : <AlertCircle className="h-4 w-4 mt-0.5 shrink-0" />}
                                <span>{status.message}</span>
                            </div>
                        ) : (
                            <div className="text-sm text-forest/50">Ready to send. Select a post below.</div>
                        )}
                    </CardContent>
                </Card>
            </div>
